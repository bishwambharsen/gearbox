...
describe('createLedger cost math', () => {
  it('computes actualCostUsd from the decision tier including cache multipliers', () => {
    ...
    ledger.record('s1', sonnetDecision, usage);
  ...
  it('computes baselineCostUsd priced on the baseline model tier, not the decision tier', () => {
  ...
  it('falls back to the most expensive tier when baselineModel matches no configured tier', () => {
  ...
  it('treats missing optional usage fields as zero', () => {
    const config = baseConfig();
    const ledger = createLedger(config);
    ledger.record('s1', sonnetDecision, { input_tokens: 10, output_tokens: 10 });
    const [entry] = readLedger(config.ledgerPath);
...
describe('createLedger + readLedger JSONL round trip', () => {
  it('appends one JSON line per record and preserves order across sessions', () => {
    const config = baseConfig();
    const ledger = createLedger(config);
    ledger.record('session-a', sonnetDecision, { input_tokens: 1, output_tokens: 1 });
    ledger.record('session-b', sonnetDecision, { input_tokens: 2, output_tokens: 2 });
    ledger.record('session-a', sonnetDecision, { input_tokens: 3, output_tokens: 3 });
  ...
  it('creates the parent directory for the ledger file if it does not exist yet', () => {
    ...
    createLedger(config).record('s1', sonnetDecision, { input_tokens: 1, output_tokens: 1 });
  ...
  it('skips malformed lines when reading', () => {
    const config = baseConfig();
    const ledger = createLedger(config);
    ledger.record('s1', sonnetDecision, { input_tokens: 1, output_tokens: 1 });

    // Simulate a corrupted line (e.g. a partial write) alongside a valid one.
    appendFileSync(config.ledgerPath, 'not valid json\n');
    ledger.record('s2', sonnetDecision, { input_tokens: 2, output_tokens: 2 });
